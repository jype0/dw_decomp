...
extern volatile short SsUtKeyOnV(short voice, short vabId, short prog, short tone, short note, short fine, short voll, short volr);
...
uint32_t startSound(int32_t vabId, char prog, char note)
{
  ...
  for (i = 0, ofs = 0; i < 0x10; i++, ofs += 0x20)
  {
    ...
    if (note == (*new_var))
    {
      ofs = (chan = getNextFreeChannel(chan));
      if (chan == (-1))
      {
        break;
      }
      SsUtKeyOnV(ofs, vabId, prog, (0, i ^ 0), note, (i < 0x10) * 0, 0x7F, 0x7F);
      mask |= 1 << ofs;
    }
  }
