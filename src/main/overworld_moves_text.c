...
int32_t drawDigimonMovesText(void)
{
	...
	case 0:
		...
		if (PARTNER_ENTITY.digimonEntity.stats.base.moves[0] != 0xff) {
			MOVES_VIEW_TECHS[0] = entityGetTechFromAnim(ENTITY_TABLE[1],
				PARTNER_ENTITY.digimonEntity.stats.base.moves[0]);
			drawString(MOVE_NAMES[MOVES_VIEW_TECHS[0]], 0, 0x18);
		} else {
			MOVES_VIEW_TECHS[0] = 0xff;
		}
		MENU_SUB_STATE = 1;
		break;
	case 1:
		...
		if (PARTNER_ENTITY.digimonEntity.stats.base.moves[1] != 0xff) {
			MOVES_VIEW_TECHS[1] = entityGetTechFromAnim(ENTITY_TABLE[1],
				PARTNER_ENTITY.digimonEntity.stats.base.moves[1]);
			drawString(MOVE_NAMES[MOVES_VIEW_TECHS[1]], 0, 0x24);
		} else {
			MOVES_VIEW_TECHS[1] = 0xff;
		}
		MENU_SUB_STATE = 2;
		DrawSync(0);
		return 0;
	case 2:
		...
		if (PARTNER_ENTITY.digimonEntity.stats.base.moves[2] != 0xff) {
			MOVES_VIEW_TECHS[2] = entityGetTechFromAnim(ENTITY_TABLE[1],
				PARTNER_ENTITY.digimonEntity.stats.base.moves[2]);
			drawString(MOVE_NAMES[MOVES_VIEW_TECHS[2]], 0, 0x30);
		} else {
			MOVES_VIEW_TECHS[2] = 0xff;
		}
		MENU_SUB_STATE = 3;
		DrawSync(0);
		return 0;
	case 3:
		if (PARTNER_ENTITY.digimonEntity.stats.base.moves[3] != 0xff) {
			MOVES_VIEW_TECHS[3] = entityGetTechFromAnim(ENTITY_TABLE[1],
				PARTNER_ENTITY.digimonEntity.stats.base.moves[3]);
			drawString(MOVE_NAMES[MOVES_VIEW_TECHS[3]], 0, 0x3c);
		} else {
			MOVES_VIEW_TECHS[3] = 0xff;
		}
